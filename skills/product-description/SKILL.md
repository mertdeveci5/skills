...
description: Create, resume, or extend an outside-in product behavior specification from code and tests, verify it against the running product, and triage suspected defects. Use when asked to describe a product's user-visible behavior feature by feature or build a product-description or behavior-spec repository; not for API docs or ordinary code review.
metadata:
  source: "https://gist.github.com/steveruizok/83ae5c53f2784ebf8f5fe0a3fb94480f"
  source-revision: "f9435a3c7b022ecadc8441542675a9163cdb6b7f"
---
...
Once the exemplars exist, the remaining documents are independent. If the harness supports subagents and the user has authorized parallel agent work, fan out one document or small cluster per agent. Otherwise draft them sequentially with the same isolation boundaries. Each agent or drafting pass uses this prompt:
